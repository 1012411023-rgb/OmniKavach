...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600/50 rounded-lg p-3 shadow-xl text-[11px]">
...
/** Custom dot for Lactate line — highlights outliers in amber */
const LactateDot = (props) => {
  ...
  return (
    <circle
      key={`dot-${cx}`}
      cx={cx} cy={cy} r={3}
      fill="#06b6d4" stroke="#0e7490" strokeWidth={1}
    />
  );
};
...
export default function PatientTimeline({ data }) {
  ...
  /* Theme-aware inline colors for SVG elements that can't use Tailwind */
  ...
  return (
    <div className="space-y-4">
      {/* B.S. Detector Banner */}
      <div className="bg-amber-50 dark:bg-amber-500/8 border border-amber-200 dark:border-amber-500/30 rounded-xl p-3 flex gap-3 shadow-sm dark:shadow-none">
        ...
        <div>
          ...
          <p className="text-amber-600/70 dark:text-amber-200/60 text-[10px] leading-relaxed">
            ...
            statistically anomalous (z-score &gt; 4.2σ). Probable hemolyzed sample or lab processing error.
